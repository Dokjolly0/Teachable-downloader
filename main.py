...
class TeachableDownloader:
    ...
    def download_course_colossal(self, course_url):
        ...
        try:
            ...
            logging.error(
                "Could not save course html: " + str(e), exc_info=self.verbose
            )
    ...
    def download_course_simple(self, course_url):
        ...
        for slim_section in slim_sections:
            ...
            try:
                not_available_element = WebDriverWait(
                    slim_section, self.global_timeout
                ).until(EC.presence_of_element_located((By.CSS_SELECTOR, ".drip-tag")))
    ...
    def download_videos_from_links(self, video_list):
        for video in video_list:
            ...
            try:
                ...
                logging.error(
                    "Could not save html: " + video["title"] + " cause: " + str(e),
    ...
    def download_video(self, link, title, video_index, output_path):
        ydl_opts = {
            ...
            "outtmpl": os.path.join(
                output_path, "{:02d}-{}.mp4".format(video_index, title)
            ),
    ...
    if args.file:
        ...
        try:
            downloader.run_batch(
                urls, args.email, args.password, args.login_url, args.man_login_url
            )
            downloader.clean_up()
            sys.exit(0)
